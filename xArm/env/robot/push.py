...
class PushEnv(BaseEnv, utils.EzPickle):
	...
	def compute_reward(self, achieved_goal, goal, info):
		object_goal = self.sim.data.get_site_xpos('object_site').copy()
	...
	def _sample_goal(self, new=True):
		...
		if new:
			...
			goal[0] += self.np_random.uniform(-scale, scale, size=1)
			goal[1] += self.np_random.uniform(-scale, scale, size=1)
		else:
	...
	def _is_success(self, achieved_goal, desired_goal):
		achieved_goal = self.sim.data.get_site_xpos('object_site')
