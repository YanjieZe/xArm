...
import copy
import xArm.env.robot.gym_utils as utils # Modified some gym utils to incorporate multiple bodies in mocap
import cv2
import io
...
class BaseEnv(robot_env.RobotEnv):
    ...
    def __init__(
        ...
        image_size=84, reset_free=False, distance_threshold=0.05, action_penalty=0,
        observation_type='image', reward_type='dense', reward_bonus=True, use_xyz=False, action_scale=0.05, 
        render=False
    ):
    ...
    def _get_obs(self):
        ...
        if self.observation_type in ['state', 'state+state']:
        ...
        return {
            ...
            'state': self._get_robot_state_obs() if self.observation_type in ['state+image','state+state'] else None
        }
    ...
    def step(self, action):
        ...
        # the following code is copied from superclass
        action = np.clip(action, self.action_space.low, self.action_space.high)
        self._set_action(action.copy())
    ...
    def render_depth(self, mode=None, width=448, height=448, camera_id=None):
        self._render_callback()
        
        data = list()
        for cam_name in self.cameras:
            data.append(self.sim.render(
                width, height, camera_name=cam_name, depth=True
            )[1][::-1, :,])
            # data.append(self.sim.render(
            #     width, height, camera_name=cam_name, depth=False
            # )[::-1, :, :])
